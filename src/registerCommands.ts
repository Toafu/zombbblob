...
export const registerCommands = async function () {
    ...
    await restClient.put(Routes.applicationGuildCommands(CLIENT_ID, SERVER_ID), { body: [] });

    console.log('Inserting new commands...');
    await restClient.put(Routes.applicationGuildCommands(CLIENT_ID, SERVER_ID), { body: commandDatums });
};
