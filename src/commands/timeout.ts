...
export const command: Command = {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    const timeoutLengthSeconds = interaction.options.getInteger(
      'timeout_length_in_sec',
      true,
    );
    ...
    if (targetMember === null) {
      await interaction.reply(
        'Failed to fetch target member, maybe they left or got kicked/banned?',
      );
      return;
    }

    const resultMessage = await targetMember
      .timeout(1000 * timeoutLengthSeconds, reason)
      .then(
        () =>
          `Timed out <@${targetUser.id}> for ${timeoutLengthSeconds} seconds`,
      )
