...
export const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      'Assigns students from start date to end date with the Roles.Student Alumni Role',
    ),
  init: (client: Client<boolean>) => {
    //For automagic detections
    const checkAlumnize = async () => {
      const beforeThreshold = new Date(); //Get today's Date
      const currMonth = beforeThreshold.getMonth();
      if (
        beforeThreshold.getDate() === 1 &&
        (currMonth === 0 || currMonth === 4 || currMonth === 6)
      ) {
        //getDate() is 1-indexed but getMonth() is 0-indexed
        if (currMonth === 0) {
          //Decrement year if checking on New Year's
        ...
        if (studentRole === null) {
          throw new Error('Could not fetch student role!');
        }

        for (const [
          studentDiscordID,
          studentMember,
        ] of studentRole.members.entries()) {
          //r.members is a Collection<userID, GuildMember>
          if (studentMember.joinedAt === null) {
            console.error(
              `Student with id ${studentDiscordID} has a null joinedAt time!`,
            );
            continue;
          }
        ...
        await serverLog.send(`Students automatically alumnized: ${count}.`);
      }
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    if (startDateArg.length !== 10 || endDateArg.length !== 10) {
      await interaction.reply(
        'Please make sure dates are of the form YYYY-MM-DD',
      );
      return;
    }
    ...
    if (
      ...
      await interaction.reply(
        'Failed to detect date. Please make sure dates are of the form YYYY-MM-DD.',
      );
      return;
    }
    let count = 0;
    ...
    for (const member of studentRole.members.values()) {
      if (member.joinedAt === null) {
        await interaction.channel.send(
          `Failed to alumnize <@${member.id}> due to null join date`,
        );
        continue;
      }
