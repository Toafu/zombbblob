...
export const command: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('start_date').setDescription('Provide a date (YYYY-MM-DD)').setRequired(true)
        )
        .addStringOption(option =>
            option.setName('end_date').setDescription('Provide a date (YYYY-MM-DD)').setRequired(true)
        )
    ...
    init: (client: Client<boolean>) => {
        ...
        const checkAlumnize = async () => {
            ...
            if (beforeThreshold.getDate() == 1 && (currMonth == 0 || currMonth == 4 || currMonth == 6)) {
                ...
                for (const [studentDiscordID, studentMember] of studentRole.members.entries()) {
