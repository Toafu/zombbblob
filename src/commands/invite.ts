...
export const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      'creates an invite with a set number of uses (default unlimited)',
    ),
  init: () => {},
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    if (numUses === null) {
      await interaction.reply('discord.gg/fnVXyhfh33');
      return;
    }

    const invite = await interaction.guild.invites.create(
      Channels.welcomeandrules,
      {maxUses: numUses, maxAge: 0},
    );
