...
export const command: Command = {
    data: new SlashCommandBuilder()
        .setName('invite')
        .addIntegerOption(option => option.setName('n').setDescription('Number of invite uses').setRequired(false))
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        const invite = await interaction.guild.invites.create(Channels.welcomeandrules, {
            maxUses: numUses,
            maxAge: 0,
        });
