...
export const command: Command = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('message_link').setDescription('The message to react to').setRequired(true)
        )
        .addStringOption(option => option.setName('reply_text').setDescription('The reply message').setRequired(true))
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        const message = await channel.messages
            .fetch(messageLinkParts.messageID)
