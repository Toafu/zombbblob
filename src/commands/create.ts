...
export const command: Command = {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    const pianoGangChannel = (await interaction.guild.channels.fetch(
      Channels.pianogang,
    )) as BaseGuildTextChannel;
    await pianoGangChannel.setParent(categoryChannel);
    await pianoGangChannel.setPosition(3);

    const oldTimersChannel = (await interaction.guild.channels.fetch(
      Channels.oldtimers,
    )) as BaseGuildTextChannel;
    await oldTimersChannel.setParent(categoryChannel);
    await oldTimersChannel.setPosition(4);

    return deferredReply.edit(
      `${categoryName} successfully created. Remember to also fix the Onboarding settings for Students.`,
    );
  },
};
