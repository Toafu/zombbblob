...
export const command: Command = {
    data: new SlashCommandBuilder()
        ...
        .addChannelOption(option =>
            option.setName('channel').setDescription('The channel to be sent in').setRequired(true)
        )
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        await targetChannel
            .send(interaction.options.getString('message', true))
            .then(_ => interaction.reply(`Message sent in <#${targetChannel.id}>`))
            .catch(_ =>
                interaction.reply("Failed to send message in that channel (I probably don't have permissions).")
            );
    },
};
