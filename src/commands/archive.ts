...
export const command: Command = {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    const [error, categoryName] = semesterStringToCategoryName(
      interaction.options.getString('semester', true).toLowerCase(),
    );
    ...
    const categoryToArchive = channels.find(
      cat =>
        cat !== null &&
    ...
    if (!categoryToArchive || !(categoryToArchive instanceof CategoryChannel)) {
      await deferredReply.edit(
        `Unable to find category called \`${categoryName}\``,
      );
      return;
    }
    ...
    await categoryToArchive.permissionOverwrites.edit(SERVER_ID, {
      SendMessages: false,
    });
    ...
    const highestNonArchiveChannel = await interaction.guild.channels.fetch(
      Channels.smallstudyrooms,
    );
    ...
    if (!(highestNonArchiveChannel instanceof CategoryChannel)) {
      await deferredReply.edit(
        'Highest non-archive channel is not a category!',
      );
      return;
    }

    await categoryToArchive.setPosition(highestNonArchiveChannel.position + 1, {
      reason: '/archive',
    });
