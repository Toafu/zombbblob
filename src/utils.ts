...
export function semesterStringToCategoryName(
  semesterString: string,
): [Error | null, string] {
  if (!/[fsw]\d{2}/.test(semesterString)) {
    return [
      new Error(
        'Invalid semester format. Must follow `[F/S/W][Last two digits of year]`',
      ),
      '',
    ];
  }
...
export function parseMessageLink(
  messageLink: string,
): [Error | null, DecodedMessageLink] {
  const messageLinkParts = messageLink.match(MESSAGE_LINK_REGEX);

  if (messageLinkParts === null) {
    return [
      new Error('Please make sure you are providing a valid message link.'),
      {guildID: '0', channelID: '0', messageID: '0'},
    ];
  }
