...
function isSpam(messageContent: String, authorID: Snowflake) {
    ...
    return (
        recentMessages.filter(x => x.content === messageContent && x.author === authorID).length >=
        MESSAGE_TIMEOUT_CRITICAL_COUNT
    );
}
...
client.on('ready', async () => {
    ...
    for (const commandFileName of fs.readdirSync(commandsDirectoryPath)) {
        const { command } = await import(path.join(commandsDirectoryPath, commandFileName));
        command.init(client);
    ...
    if (reaction.message.id === updateRoleMessage) {
        ...
        await guild.members.fetch(user.id).then(async member => {
            ...
            if (reaction.emoji.name === '🧠') {
                // '\u{0001F9E0}'
                guild.roles.fetch(Roles.StudentAlumni).then(r => {
                    if (r === null) {
                ...
                //reaction.emoji.name === '🎓'
                guild.roles.fetch(Roles.Student).then(r => {
                    if (r === null) {
                        console.error(`Failed to fetch Student Role for ${user.id}`);
                        return;
                    }
